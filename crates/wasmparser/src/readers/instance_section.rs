...
impl<'a> InstanceSectionReader<'a> {
    ...
    pub fn read(&mut self) -> Result<Instance<'a>> {
        let instance =
            Instance::new(&self.reader.buffer[self.reader.position..], self.original_position())?;
...
impl<'a> SectionReader for InstanceSectionReader<'a> {
    ...
    fn read(&mut self) -> Result<Self::Item> { InstanceSectionReader::read(self) }

    fn eof(&self) -> bool { self.reader.eof() }

    fn original_position(&self) -> usize { InstanceSectionReader::original_position(self) }

    fn range(&self) -> Range { self.reader.range() }
}

impl<'a> SectionWithLimitedItems for InstanceSectionReader<'a> {
    fn get_count(&self) -> u32 { InstanceSectionReader::get_count(self) }
}
...
impl<'a> Instance<'a> {
    pub fn new(data: &'a [u8], offset: usize) -> Result<Instance<'a>> {
        let mut reader = BinaryReader::new_with_offset(data, offset);
        if reader.read_u8()? != 0 {
            return Err(BinaryReaderError::new("instantiate instruction not found", offset));
        }
...
impl<'a> SectionReader for InstanceArgsReader<'a> {
    ...
    fn read(&mut self) -> Result<Self::Item> { InstanceArgsReader::read(self) }

    fn eof(&self) -> bool { self.remaining == 0 }

    fn original_position(&self) -> usize { InstanceArgsReader::original_position(self) }
