...
impl<'a> SectionReader for LinkingSectionReader<'a> {
    ...
    fn read(&mut self) -> Result<Self::Item> { LinkingSectionReader::read(self) }

    fn eof(&self) -> bool { self.reader.eof() }

    fn original_position(&self) -> usize { LinkingSectionReader::original_position(self) }

    fn range(&self) -> Range { self.reader.range() }
}

impl<'a> SectionWithLimitedItems for LinkingSectionReader<'a> {
    fn get_count(&self) -> u32 { LinkingSectionReader::get_count(self) }
}
