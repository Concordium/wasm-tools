...
impl<'a> ProducersSectionReader<'a> {
    ...
    /// use wasmparser::{ProducersFieldValue, ProducersSectionReader, Result};
    ...
    /// let value =
    ///     values_reader.into_iter().collect::<Result<Vec<ProducersFieldValue>>>().expect("values");
    /// assert!(value.len() == 2);
...
impl<'a> SectionReader for ProducersSectionReader<'a> {
    ...
    fn read(&mut self) -> Result<Self::Item> { ProducersSectionReader::read(self) }

    fn eof(&self) -> bool { self.reader.eof() }

    fn original_position(&self) -> usize { ProducersSectionReader::original_position(self) }

    fn range(&self) -> Range { self.reader.range() }
}

impl<'a> SectionWithLimitedItems for ProducersSectionReader<'a> {
    fn get_count(&self) -> u32 { ProducersSectionReader::get_count(self) }
}
