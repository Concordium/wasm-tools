use crate::{
    operators_validator::OperatorValidator, BinaryReader, FunctionBody, Operator, Result, Type,
...
impl<T: WasmModuleResources> FuncValidator<T> {
    ...
    pub fn operand_stack_height(&self) -> u32 { self.validator.operands.len() as u32 }
    ...
    pub fn resources(&self) -> &T { &self.resources }
}
...
mod tests {
    ...
    impl WasmModuleResources for EmptyResources {
        ...
        fn table_at(&self, _at: u32) -> Option<crate::TableType> { todo!() }

        fn memory_at(&self, _at: u32) -> Option<crate::MemoryType> { todo!() }

        fn event_at(&self, _at: u32) -> Option<&Self::FuncType> { todo!() }

        fn global_at(&self, _at: u32) -> Option<crate::GlobalType> { todo!() }

        fn func_type_at(&self, _type_idx: u32) -> Option<&Self::FuncType> { Some(&EmptyFuncType) }

        fn type_of_function(&self, _func_idx: u32) -> Option<&Self::FuncType> { todo!() }

        fn element_type_at(&self, _at: u32) -> Option<Type> { todo!() }

        fn element_count(&self) -> u32 { todo!() }

        fn data_count(&self) -> u32 { todo!() }

        fn is_function_referenced(&self, _idx: u32) -> bool { todo!() }
    }

    struct EmptyFuncType;

    impl WasmFuncType for EmptyFuncType {
        fn len_inputs(&self) -> usize { 0 }

        fn len_outputs(&self) -> usize { 0 }

        fn input_at(&self, _at: u32) -> Option<Type> { todo!() }

        fn output_at(&self, _at: u32) -> Option<Type> { todo!() }
    }
